...
function normalizeEpicGame(originalTitle, item, prices) {
  ...
  const categories = Array.isArray(item.categories)
    ? item.categories.map((category) => category.path).filter(Boolean).slice(0, 6)
    : [];
...
function mergeGames(originalTitle, steamGame, epicGame, appId) {
  ...
  return {
    ...
    cover: primary.cover || "",
    cardImage: primary.cardImage || primary.header || primary.cover || "",
    header: primary.header || "",
    portrait: primary.portrait || primary.cover || "",
    hero: primary.hero || primary.header || "",
    screenshots,
